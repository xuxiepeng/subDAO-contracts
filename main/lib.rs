...
mod main {
    ...
    impl Main {
        ...
        pub fn init(
            &mut self,
            template_code_hash: Hash,
            user_manager_code_hash: Hash,
            salt: Vec<u8>,
        ) -> bool {
            ...
            let contract_addr =
                init_result.expect("failed at instantiating the `TemplateManager` contract");
            ...
            let user_manager_instance_params = UserManager::new()
                ...
                .salt_bytes(&salt)
                .params();
            let user_manager_init_result =
                ink_env::instantiate_contract(&user_manager_instance_params);
            let user_manager_contract_addr = user_manager_init_result
                .expect("failed at instantiating the `UserManager` contract");
            let user_manager_contract_instance =
                ink_env::call::FromAccountId::from_account_id(user_manager_contract_addr);
        ...
        pub fn add_template(
            ...
            components: BTreeMap<String, Hash>,
        ) -> bool {
            self.template
                .as_mut()
                .unwrap()
                .add_template(name, dao_manager_code_hash, components)
        }
        ...
        #[ink(message)]
        pub fn query_user_manager_addr(&self) -> AccountId {
            self.user_manager_addr.unwrap()
        }

        #[ink(message)]
        pub fn instance_by_template(
            &mut self,
            index: u64,
            ...
            let dao_addr =
                dao_init_result.expect("failed at instantiating the `DAO Instance` contract");
            let mut dao_instance: DAOManager =
                ink_env::call::FromAccountId::from_account_id(dao_addr);
        ...
        pub fn list_dao_instances(&mut self, page: u64, size: u64) -> PageResult<DAOInstance> {
            ...
            for i in start..end {
                let key = self.instance_map.keys().nth(i as usize);
                if let Some(s) = key {
                    dao_vec.push(Main::fill_dao_details(
                        self.instance_map.get(s).unwrap().clone(),
                    ));
                }
            }
        ...
        #[ink(message)]
        pub fn list_dao_instances_by_owner(
            &mut self,
            owner: AccountId,
            page: u64,
            size: u64,
        ) -> PageResult<DAOInstance> {
            ...
            if id_list_op.is_some() {
                ...
                for i in start..end {
                    ...
                    if let Some(s) = key {
                        let dao: DAOInstance =
                            Main::fill_dao_details(self.instance_map.get(s).unwrap().clone());
                        dao_vec.push(dao);
                    }
                }
        ...
        #[ink(message)]
        pub fn list_dao_instances_by_account(
            &mut self,
            user: AccountId,
            page: u64,
            size: u64,
