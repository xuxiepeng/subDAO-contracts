        ...
        pub fn list_dao_instances_by_account(
            ...
            for elem in self.instance_map.keys() {
                if let Some(dao) = self.instance_map.get(elem) {
                    ...
                    let org_addr: AccountId = org_addr_op.unwrap();
                    let org_instance: OrgManager =
                        ink_env::call::FromAccountId::from_account_id(org_addr);
                    let (is_member, is_moderator, is_owner) =
                        org_instance.check_role_by_account(user);
